...
import { validatePhoneNumber, validateBulkPhoneNumbers } from '../services/phoneValidationService';
import { BulkValidationResult, PhoneValidationResult, CSVValidationResult } from '../types/phoneValidation';
...
export class PhoneValidationController {
  ...
  /**
   * Validate multiple phone numbers from JSON array
   */
  async validateBulk(req: AuthRequest, res: Response) {
    try {
      ...
      if (phoneNumbers.length > 1000) {
        return res.status(400).json({ error: 'Maximum 1000 phone numbers allowed per request' });
      }
      ...
      res.status(500).json({ error: 'Failed to validate phone numbers', details: error.message });
    }
  }

  /**
   * Validate phone numbers from CSV file
   */
          ...
          .on('end', async () => {
            try {
              if (phoneNumbers.length === 0) {
                return res.status(400).json({ error: 'No phone numbers found in CSV file' });
              }

              if (phoneNumbers.length > 10000) {
                return res.status(400).json({ error: 'Maximum 10000 phone numbers allowed per CSV file' });
              }
              ...
              const csvResults: Array<{ row: number; phoneNumber: string; result: any }> = csvData.map((csvRow, index) => ({
                row: csvRow.row,
                phoneNumber: phoneNumbers[index] || '',
                result: minimal[index] || {
                  phoneNumber: phoneNumbers[index] || '',
                  isValid: false
                }
              }));
